...
export function buildProductFittingPrompt(
  ...
  return [
    ...
    }${colorOverrideNote?.trim() ? '' : ' is the actual product color being sold — match it precisely, do not shift the hue, saturation, or brightness. Do NOT pull color from any other image or from the texture description.'} CONTRAST-TRIM EXCEPTION: this "one main color" rule governs the garment\'s MAIN body fabric only. If the Details / construction map above explicitly names a SECOND contrast color on a specific part (e.g. a two-tone rolled-up cuff revealing a grey inner fabric, a contrast-colored collar band, ringer trim, or a contrast panel), that part is genuinely that stated second color and MUST be rendered in it — do NOT recolor a contrast accent toward the main body color (e.g. a grey rolled cuff must stay GREY, not be tinted blue to match a navy body). Only the explicitly-named contrast parts get the second color; everything else stays the main color.`,
    ...
    `- CRITICAL SEAM/POCKET/PATCH RULE: the "Details" spec above lists the exact seam/panel lines, pocket type+location, and logo/patch placement AND APPEARANCE found by directly inspecting the real product photos. Treat this list as a checklist — reproduce each item at its stated location, in the stated quantity, and invent NOTHING beyond what is listed (no extra pocket, no extra patch, no seam line that isn't described). A single patch mentioned once must appear exactly once, at the location described — never mirrored onto both sides or duplicated. The patch/logo's own COLOR, SHAPE, and SIZE must match the spec (e.g. a small navy woven rectangular label stays a small navy rectangle in the same spot). SMALL-TEXT REALISM (important — brand text on a small woven label cannot be reproduced letter-accurately at this scale, and forcing it produces confident WRONG lettering that looks fake): render the label as a real small woven/printed patch with the correct background color, shape, and placement, and with fine text that reads as natural, softly-resolved label lettering at photographic scale — do NOT stamp large, bold, sharply-legible invented words or a fake brand name onto it. If the spec describes multiple lines of fine print (a small brand story, "since [year]", care text, etc.), render those lines as very light, subtle, low-contrast soft micro-texture on the label's background — barely-there suggestion of text, NOT dark, blotchy, ink-stain-like marks. A clean, mostly-plain label with one soft legible line beats a smudged, dirty-looking label with attempted fine print — if in doubt, simplify toward a cleaner label rather than adding more illegible marks. The patch must always look like a crisp, clean, unstained fabric label, never like a stain, smudge, or dirt mark on the pocket.`,
    // (2026-07-21) 상의/아우터 전용 구조 지도 — 목/소매/밑단. 이 세 부위는 마감(골지 밴드/일반
    // 시접)과 대조 트림 유무가 서로 달라서 가장 자주 틀린다(실제 사고: 목·소매엔 흑백 휘프스티치,
    // 밑단은 대조 스티치 없는 골지 밴드인데 뭉개짐). 하의용 다리/포켓 행은 상의에 무의미하므로 빼고
    // 이 세 존만 좌표처럼 고정한다.
    // (2026-07-27) 예전엔 `!== 'bottom'` 이라 신발·액세서리도 이 "상반신 옷 목/소매/밑단" 지도를
    // 받았는데, 신발한테 "NECKLINE: none, SLEEVE CUFFS: none"을 먹이는 꼴이라 무의미한 노이즈였다.
    // 상반신 착의(top/outer)로만 한정한다. 신발·액세서리의 비대칭/디테일은 아래 공용 ASYMMETRY 줄과
    // Details·SEAM/POCKET/PATCH 규칙(카테고리 무관)이 담당한다.
    ...(garmentAnalysis.constructionMap && (category === 'top' || category === 'outer')
      ? [
          [
    ...
    ...(garmentAnalysis.constructionMap && category === 'bottom'
      ? [
          [
            ...
            `  OUTER-THIGH / CARGO POCKET EXCEPTION (important): a cargo pocket, hammer/tool loop, or patch described as being on the OUTER THIGH or SIDE of the leg (as opposed to strictly a front-fly-area or strictly a back-patch-pocket-area detail) sits on the SIDE of the leg and stays visible in front, back, AND side/turned views alike — a person does not lose their cargo pocket by turning around. If such a detail is filed under a FRONT-leg line but the current pose is back-facing (so the BACK-leg line for that same leg says "none"), still render that outer-thigh detail on its stated leg exactly as described — it is the same physical pocket/loop, just now seen from behind, not a detail that disappears. This applies per-leg independently: a loop stated for one leg and a patch stated for the other leg must land on their own separate, correct legs even when viewed from the back.`,
      ...
      : []),
    // (2026-07-27) 비대칭 체크리스트를 하의 외 전 카테고리(상의/아우터/신발/액세서리)에도 노출한다.
    // 예전엔 이 정보가 하의 블록 안에서만 프롬프트로 나가서, 한쪽 가슴 로고 / 한쪽 소매 패치 /
    // 한쪽 신발 바깥면 로고 / 한쪽 어깨 가방 같은 비대칭을 분석은 뽑아놓고도 생성기에 안 넘겼다.
    // 신발/액세서리는 위 EDGE 지도를 안 받으므로 이 줄이 비대칭에 대한 주된 신호가 된다.
    ...(garmentAnalysis.constructionMap &&
    category !== 'bottom' &&
    !/^none\b/i.test(garmentAnalysis.constructionMap.asymmetryChecklist.trim())
      ? [
          `- ASYMMETRY CHECKLIST (one-side-only details — the single most commonly failed part of this task): ${garmentAnalysis.constructionMap.asymmetryChecklist}\n  ASYMMETRY RENDERING RULE: each item exists on EXACTLY ONE side of the product (e.g. a logo on the outer side of ONLY one shoe, a patch on ONLY one sleeve/chest side, a charm on ONLY one side of a bag). Render it on the stated side ONLY, then verify the opposite side is plain — never mirror or duplicate it. Apply the mirror rule when reading the reference photo: in a FRONT view the wearer's LEFT appears on the VIEWER'S RIGHT; in a BACK view it does not mirror. If it says "none / symmetric", keep both sides identical and do NOT invent any one-sided logo, patch, or mark.`,
        ]
      : []),
    `- CRITICAL FRONT/BACK CONSISTENCY RULE: real garments look different from the front and back. Use garment anatomy to tell them apart in the reference photos — for BOTTOMS, the FRONT is the side with the center fly (button/zipper) and slanted side-entry pockets, and the BACK is the side with patch pockets/yoke/elastic gathering and NO fly; for TOPS, the front has the placket/graphic/forward-opening collar. Then render ONLY the side that matches the pose: a front-facing pose must show the fly and must NOT show back patch pockets wrapping around to the front; a back-facing pose must show the back construction and no fly. Never combine front details and back details into a single view, and never guess at an unseen side beyond a plain, undecorated continuation. EXCEPTION (does not count as "combining front and back"): a cargo pocket, hammer/tool loop, or patch on the OUTER THIGH/SIDE of the leg (per the OUTER-THIGH / CARGO POCKET EXCEPTION above, if a CONSTRUCTION MAP is present) is a side-of-leg feature visible from every angle — rendering it in a back-facing shot is showing a real, always-visible part of the garment, NOT inventing or borrowing a front-only detail.`,
    '',
    '=== POSE & FRAMING (ABSOLUTE) ===',
