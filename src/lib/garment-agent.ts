...
4. MANDATORY STRUCTURAL SCAN — before writing "details", actually look at the garment piece by piece and identify EVERY one of the following if present, with its exact location (e.g. "left thigh", "back right pocket", "center chest", "outer side seam"):
   ...
   - Any logo, brand patch, embroidery, print, or label — exact placement AND exact appearance, described as TWO SEPARATE observations so you don't guess one from the other: (1) the patch/label's OWN BACKGROUND FABRIC color (the color the patch itself is made of/dyed — commonly cream, off-white, tan, or a solid color; look at the patch's edges/corners where there is no text, that is the pure background color), and (2) the color of the TEXT/LOGO/GRAPHIC printed or woven ON TOP of that background (commonly navy, black, or a contrasting color) — these two colors are usually DIFFERENT from each other, so do not default to assuming a "colored background + white text" pattern; check each independently. Also note the LAYOUT honestly: many woven labels are NOT a single word/logo — they often have multiple lines of small fine-print text (e.g. a brand story, "since [year]", care text) above or below a smaller cursive/script wordmark or symbol; do not flatten a multi-line vintage-style label into a single bold word. State shape (rectangular/round/shield) and exact placement.
